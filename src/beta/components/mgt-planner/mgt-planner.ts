...
import { MgtFlyout } from '../../../components/sub-components/mgt-flyout/mgt-flyout';
...
export class MgtPlanner extends MgtTasksBase {
  ...
  protected renderNewTaskDetails(): TemplateResult {
    ...
    const buckets = this._buckets.filter(
      bucket =>
        (this._currentPlan && bucket.planId === this._currentPlan) ||
  ...
  private async loadAllTasks(): Promise<void> {
    ...
    const buckets = bucketResults.reduce((cur, ret) => [...cur, ...ret], []);

    const taskResults = await Promise.all(buckets.map(b => getPlannerTasks(this._graph, b.id)));
  ...
  private getPeoplePicker(task: GraphTypes.PlannerTask): Components.MgtPeoplePicker {
    const taskId = task ? task.id : 'newTask';
    const picker = this.renderRoot.querySelector(`.picker-${taskId}`) as Components.MgtPeoplePicker;

    return picker;
  }

  private getMgtPeople(task: GraphTypes.PlannerTask): Components.MgtPeople {
    const taskId = task ? task.id : 'newTask';
    const mgtPeople = this.renderRoot.querySelector(`.people-${taskId}`) as Components.MgtPeople;
